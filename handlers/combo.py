...
from utils.messages import approved_message, charged_message, insufficient_funds_message, dato, hit_detected_message
...
📄 File : {message.document.file_name}
        ...
        wait_msg = bot.send_message(message.chat.id, "<b>⏳ PROCESSING YOUR COMBO FILE...</b>", parse_mode="HTML")
        ...
        try:
            ...
            bot.edit_message_text(f"<b>❌ FAILED TO PROCESS FILE: {e}</b>", message.chat.id, wait_msg.message_id, parse_mode="HTML")
            return

        if not cards:
            bot.edit_message_text("<b>❌ EMPTY FILE</b>", message.chat.id, wait_msg.message_id, parse_mode="HTML")
            return

        # إنشاء Session جديدة
        sessions[uid] = ComboSession(cards, message.document.file_name)
        sessions[uid].checking = False  # تأكد من reset
        ...
        bot.edit_message_text("<b>ϟ CHOOSE THE GATEWAY ϟ</b>", message.chat.id, wait_msg.message_id, reply_markup=kb, parse_mode="HTML")
    ...
    def start_check(c):
        ...
        # تعديل: فقط خصم النقاط إذا Gate نوعه CHARGE
        required_credits = cost * total if gate_type != "AUTH" else 0

        with user_locks[uid]:
            if not is_admin(uid) and get_credits(uid) < required_credits:
        ...
        executor.submit(run_check, uid, chat_id, message_id, gate_key, total, required_credits, user_name)
...
def run_check(uid, chat_id, message_id, gate_key, total, cost, user_name):
    ...
    try:
        for i, card in enumerate(session.cards[:total]):
            ...
            # تحقق من الكريدت فقط لـ CHARGE
            if not is_admin(uid) and gate_type != "AUTH" and get_credits(uid) < cost:
                session.stop = True
                try:
            ...
            # خصم النقاط فقط لـ CHARGE
            if not is_admin(uid) and gate_type != "AUTH" and "error" not in r_text.lower():
                with user_locks[uid]:
        ...
        if uid in sessions:
            del sessions[uid]    @bot.message_handler(content_types=["document"])
    def receive_combo(message):
        uid = message.from_user.id
